            ...
            if product.amount_stock == None or item.amount < product.amount_stock:
                ...
                if product.amount_stock:
                    if quantity > product.amount_stock:
                        ...
                        item.save()
                        messages.add_message(request=request, level=messages.SUCCESS, message="No puede agregar más de las unidades que hay disponibles.", extra_tags="alert-add-cart")
                    else:
                    ...
                    if quantity > 10:
                        ...
                        messages.add_message(request=request, level=messages.SUCCESS, message="Solo puede agregar 10 como máximo para este producto.", extra_tags="alert-add-cart")         
                    else:
                        item.amount += quantity_product
                        item.subtotal += product.price * quantity_product
                        item.save()
